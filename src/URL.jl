...
function convert_dict(url::URL)
	d = OrderedDict{String, Any}()

	for field in fieldnames(URL)
		f = string(field)
		startswith(f, "_") && continue
		value = getfield(url, field)
		if !isempty(value)
			d[f] = value
		end
	end

	qp = url.query |> QueryParams
	qv = url.query |> QueryParamsValues
	qps = url.query |> QueryPairs

	!isempty(qp) && (d["query_params"] = qp)
	!isempty(qv) && (d["query_values"] = qv)
	!isempty(qps) && (d["query_pairs"] = qps)

	if !isempty(url.subdomain)
		d["subdomain_combination"] = SubCombination(url.subdomain)
	end
